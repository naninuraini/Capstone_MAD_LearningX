...
class _TambahJadwalState extends State<TambahJadwalView> {
  final TambahJadwalController controller = Get.find<TambahJadwalController>();
  late Jadwal jadwal;
  ...
  void initState() {
    ...
    if (userId.isEmpty) {
      // Jika userId kosong, beri peringatan bahwa pengguna belum login
      Get.snackbar('Error', 'Anda belum login. Silakan login terlebih dahulu.');
      // Navigasi kembali atau redirect ke halaman login
      Get.offNamed('/login'); // Ganti dengan rute halaman login yang sesuai
    }
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      ButtonWidget(
                        title: 'Upload',
                        onPressed: () {
                          if (jadwal.myUser.id.isEmpty) {
                            Get.snackbar('Error',
                                'User belum login atau ID tidak valid');
                            return;
                          }

                          if (controller.formKeyTambahJadwal.currentState!
                              .validate()) {
                            setState(() {
                              jadwal.tanggalDanWaktu =
                                  controller.selectedDateTime!;
                              jadwal.jumlah =
                                  int.parse(controller.jumlahController.text);
